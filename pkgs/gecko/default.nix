...
, perl, unzip, zip, gnumake, yasm, pkg-config, xorg, gnome2, pango, freetype, fontconfig, cairo
...
let
  ...
  buildInputs = [
    ...
    perl unzip zip gnumake yasm pkg-config
  ...
  fhs = buildFHSUserEnv rec {
    ...
    profile = ''
      ...
      export PKG_CONFIG_PATH=/usr/lib/pkg-config:/usr/share/pkg-config
      ${shellHook}
    '';
  };
in
